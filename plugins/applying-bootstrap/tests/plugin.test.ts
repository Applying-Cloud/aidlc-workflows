// plugins/applying-bootstrap/tests/plugin.test.ts — the applying-bootstrap plugin's OWN test harness.
//
// Validates the plugin's own content with the same rigor the framework applies
// to core: every stage passes validateStageFrontmatter, agents resolve against
// the real roster, produced artifacts are correctly namespaced, and every
// contribution targets a real core stage.
//
// Run:  bun test plugins/applying-bootstrap/tests/plugin.test.ts
...
const PLUGIN_NAME = "applying-bootstrap";
    ...
    for (const file of pluginStageFiles) {
      ...
      test(`${name} has a non-empty body`, () => {
        ...
        if (body.trim().length === 0) {
          throw new Error(
            `${name}: stage body is empty - the stage is behaviorally dead`
          );
        }
